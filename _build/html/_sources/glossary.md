...
Drainage area
  The upstream area that contributes to discharge by collecting rainfall. The drainage area can be quantified at any point of the landscape from the geometry (height) of the topography. Several algorithms are used to extract it from a DEM (Digital Elevation Model) by making different assumption on how the flow of water accumulates from one point of the DEM to its neighbour(s).
